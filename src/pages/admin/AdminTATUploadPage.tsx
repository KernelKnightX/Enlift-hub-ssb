...
import { saveTATSet } from '@/lib/testDataService';
import { uploadTATImage } from '@/lib/firebaseStorage';
...
export default function TATUploadPage() {
      ...
      try {
        ...
        const parsedImages: ImageEntry[] = json.map((item: unknown, index: number) => {
          ...
          return {
            id: `${index + 1}`,
            url: itemObj.url || '',
            alt: itemObj.alt || `TAT Image ${index + 1}`
          };
        });
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        if (img.file) {
          ...
          const url = await uploadTATImage(img.file, setId, i + 1);
          uploadedImages.push({
            id: `${i + 1}`,
            url,
            alt: img.alt || `TAT Image ${i + 1}`
          });
        } else {
          // Use existing URL
          uploadedImages.push({
      ...
      await saveTATSet(setId, uploadedImages);
  ...
  const downloadTemplate = () => {
    const template = [
      { url: 'https://example.com/tat1.jpg', alt: 'TAT Image 1' },
      { url: 'https://example.com/tat2.jpg', alt: 'TAT Image 2' }
    ];
    const blob = new Blob([JSON.stringify(template, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'tat_template.json';
    a.click();
      ...
      <div className="bg-white border-b px-6 py-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold">Upload TAT Images</h1>
              <p className="text-sm text-muted-foreground">Manage Thematic Apperception Test images</p>
            </div>
          </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full p-3 border-2 border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 bg-background"
                >
